...
if input_file:
    ...
    print(input_files)
    print(entrypoint)
    print(create_work_args)
    '''
    if create_work_args is None: create_work_args=dict()
    '''
    create_work_args['target_nresults'] = 1
    create_work_args['min_quorum'] = 1
    create_work_args['max_error_results'] = 1
    '''
    ...
    try:
        ...
        try:
            ...
            if 'No such object'.encode() in e.output:
        ...
        extra_opts = '\n'.join([' '*4+('<{0}>{1}</{0}>'.format(*next(iter(i.items()))) if isinstance(i,dict) else '<%s/>'%i)
        ...
        #input_files.append(("shared/test.txt",("test.txt", "3 6"),[]))
        ...
        '''
        input_files.append(("shared/main.py", "main.py", []))
        input_files.append(("shared/model.py", "model.py", []))
        input_files.append(("shared/nodeLogger.py", "nodeLogger.py", []))
        input_files.append(("shared/cfg_1.json", "cfg_1.json", []))
        input_files.append(("shared/sharedModel.h5", "sharedModel.h5", []))
        '''
...
def get_image_size(image):
    ...
    """
    output = check_output("docker images --format '{{ .Size }}' "+image,shell=True,stderr=STDOUT).decode('utf-8').splitlines()
    if len(output)==0: 
...
def escape_string(s):
    ...
    """
    return check_output(["bash","-c",'printf "%q" "$@"','_', s]).decode('utf-8')
